...
pub fn back_to_origin() -> Result<()> {
    ...
    let origin_path = storage.get_worktree_origin(&repo_name, &branch_name)?
    ...
    if !origin_pathbuf.is_dir() {
        anyhow::bail!(
            "Origin path is not a directory: {}",
            origin_path
        );
    }
...
fn determine_current_worktree(current_dir: &std::path::Path, storage: &WorktreeStorage) -> Result<(String, String)> {
    ...
    let storage_root = storage.get_root_dir().canonicalize()
    ...
    if let Ok(relative_path) = canonical_current.strip_prefix(&storage_root) {
        ...
        if components.len() >= 2 {
            ...
            let original_branch = storage.get_original_branch_name(&repo_name, &sanitized_branch)?
