# Feature: Post-Create Hooks

## Purpose

The `.worktree-config.toml` configuration file supports an `[on-create]` section that defines shell commands to run automatically after a worktree is created. This enables project-specific setup steps (e.g., installing dependencies, trusting tool configs) to run without manual intervention.
