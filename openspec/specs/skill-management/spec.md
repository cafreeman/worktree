# Specification: Skill Management

## Purpose

Provides commands for installing, uninstalling, updating, and reporting the status of the bundled Claude agent skill. The skill file is embedded in the binary at compile time and managed under `~/.agents/skills/worktree-manager/` with a symlink from `~/.claude/skills/worktree-manager`.
